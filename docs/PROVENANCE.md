...
reconstructed from the interactive eelgrass analysis session, then refactored
into library form. The provider/report layer was lifted from MethylAsk. Cell
numbers below refer to that session's execution order.

| bio-core module | Source | Recovered/lifted from |
|---|---|---|
| `methylation/model.py` | eelgrass agg.awk | weighted-methylation formula Σn_mod/Σ(n_mod+n_canonical), cov>=5; context CG/CHG/CHH (cells ~12-38) |
